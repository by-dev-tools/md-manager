...
- **CI gates + Dependabot (PR pending push, branch `ci-setup`)** — Three parallel jobs (typecheck/build/test) on `pull_request` and `merge_group` events; concurrency scoped by event-name + ref; workflow-level `permissions: { contents: read }`. Dependabot configured for npm with version-updates suppressed (security updates flow via repo settings). Unblocks the merge queue gate. FB-0020, FB-0021 captured. 2026-05-14.
