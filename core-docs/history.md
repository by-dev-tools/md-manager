...
- **Plugin rough edges stay in plugin-side feedback, not consumer-side.** Caught a handful during Phases 4.3 + 4.4 — none landed in md-manager's `core-docs/feedback.md`; all queued for a separate `by-dev-tools/flow` PR (see CLAUDE.md § "Flow plugin (in-migration)" for the canonical rule).

**Retro /critique-plan + /simplify findings (added after PR open):**

Both steps were skipped during execution — workflow violation per `.claude/rules/general.md` ("Run /simplify after commit, before /staff-review … not optional"). Retro-ran after PR open at user direction; findings captured here so future readers see what the original plan missed.

`/critique-plan` (via `assumption-auditor:plan-critic`): **REDIRECT** verdict. Two load-bearing findings:
- **Missing "base is current with `origin/main`" assumption.** The plan's three confidence verdicts did not name base-currency as load-bearing. Branch forked at `b8b0e0b`; `origin/main` had advanced to `eb8e7b9` by Phase 4.3 time. A one-line `git fetch && git merge-base --is-ancestor origin/main HEAD` preflight would have caught this; `/flow:staff-review` instead burned 4 lens spawns converging on it as the headline BLOCKER. Run a base-currency preflight before any diff-reading step on a long-lived branch.
- **HIGH miscalibrated on namespace coexistence.** The plan itself noted user-scope settings staleness (`assumption-auditor@llm-auditor` from before flow's rename) but still rated namespace-coexistence HIGH. Should have been MEDIUM with a `/plugin marketplace list | grep '^flow'` precheck. Marketplace-key-mismatch silent failure later surfaced as the Phase 4.2 BLOCKER that prompted commit `76b4653` (marketplace-prerequisite CLAUDE.md amendment).

`/simplify` (via general-purpose code review): **NIT-ONLY** verdict. 4 small polish items — rough-edges-rule restated in 3 files (trimmed to CLAUDE.md cross-reference); "13/14" vs "13 of 14" drift (normalized to "13 of 14"); `$comment-rustWorkspaceDir-omitted` value 110 chars (tightened to "Web stack — Tauri/Rust-only per schema. Intentional."); CLAUDE.md prereq/verify/silent-failure trio could collapse (declined — current shape is scannable). First three landed in the same commit as this augment; fourth deferred as taste-not-improvement.

**What this would have changed about execution.** Running `/critique-plan` upfront would have prompted the base-currency preflight that prevented the stale-base 4-lens-spawn discovery loop. Running `/simplify` upfront would have caught the duplications + terminology drift before they shipped. Both add minutes; the cost of skipping is hours. Captured in feedback.md as FB-0033 for durable enforcement.
